...
class Advanced_Visit_Counter_Queries
{
    ...
    public function apvc_sanitize_text_string( $input )
    {
        // Remove "type=" and any attributes after it
        $sanitized_input = preg_replace( '/\\btype\\s*=\\s*("[^"]*"|\'[^\']*\'|[^"\'>\\s]*)/i', '', $input );
        $pattern = '/type=image\\s+src=1\\s+onerror=alert\\(1\\)>/i';
        $sanitized_input = preg_replace( $pattern, '', $sanitized_input );
        // Use str_replace to remove the specific string and variations
        $sanitized_input = str_replace( [ 'type=image src=1 onerror=alert(1)>', 'type=image src=1 onerror=alert(1)&gt;', 'type=image src=1 onerror=alert(1)&gt;' ], '', $sanitized_input );
        $sanitized_input = str_replace( [ 'src=1 onerror=alert(1)>', 'type=image src=1 onerror=alert(1)&gt;', 'type=image src=1 onerror=alert(1)&gt;' ], '', $sanitized_input );
        $sanitized_input = str_replace( [ 'onerror=alert(1)>', 'type=image src=1 onerror=alert(1)&gt;', 'type=image src=1 onerror=alert(1)&gt;' ], '', $sanitized_input );
        // Optionally, trim any leading or trailing whitespace
        $sanitized_input = trim( $sanitized_input );
        return sanitize_text_field( $sanitized_input );
    }
    ...
    public function apvc_save_settings()
    {
        ...
        $formData = $this->apvc_sanitize_text_string( $_POST['formData'] );
        $default_label = $this->apvc_sanitize_text_string( $_POST['default_label'] );
        $todays_label = $this->apvc_sanitize_text_string( $_POST['todays_label'] );
        $global_label = $this->apvc_sanitize_text_string( $_POST['global_label'] );
        $range_posts = $this->apvc_sanitize_text_string( $_POST['range_posts'] );
        $range_ip_add = $this->apvc_sanitize_text_string( $_POST['range_ip_add'] );
        $ex_show_counter = $this->apvc_sanitize_text_string( $_POST['ex_show_counter'] );
