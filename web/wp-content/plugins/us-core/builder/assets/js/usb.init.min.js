...
var html=''+res.data.html;html=html.replace(/(us_animate_this)/g,"$1 start");if(isUpdateIncludeParent){$usb.postMessage('updateSelectedElm',[parentId,html])}else{$usb.postMessage('insertElm',[id,position,html]);$usb.postMessage('maybeInitElmJS',newId)}
...
$usbcore.copyTextToClipboard($ush.toString(self.getElmShortcode(id)).replace(/\susbid="([^\"]+)"/gi,''))},_elmDelete:function(removeId){var self=this;if(!self.isValidId(removeId)){return}
...
return!1},getShortcodePattern:function(tag){return new RegExp('\\[(\\[?)('+tag+')(?![\\w-])([^\\]\\/]*(?:\\/(?!\\])[^\\]\\/]*)*?)(?:(\\/)\\]|\\](?:([^\\[]*(?:\\[(?!\\/\\2\\])[^\\[]*)*)(\\[\\/\\2\\]))?)(\\]?)','g')},removeHtmlWrap:function(content){return(''+content).replace(/^<[^\[]+|[^\]]+$/gi,'')},parseShortcode:function(shortcode){var self=this;if(!shortcode){return{}}
...
return!!name&&/^vc_tta_(tabs|tour|accordion|section)$/.test(name)},isElmTab:function(name){var self=this;if(self.isValidId(name)){name=self.getElmType(name)}
...
this._changeField(selectedElmId,designField,designField.getValue(),!0)},__updateColumnsLayout:$ush.debounce(function(id,layout){$usb.builder._updateColumnsLayout(id,layout)},1),__updateShortcode:$ush.throttle($ush.fn,1,!0),__updateShortcode_long:$ush.debounce($ush.fn,150),__updateOnInstructions_long:$ush.throttle($ush.fn,1000),_changeField:function(selectedElmId,usofField,_,_skipSave){var self=this;self.$$fieldsets.autoShowingTabs();if(!selectedElmId||selectedElmId!==$usb.builder.selectedElmId){return}
...
if(hasChanged&&_currentData.elmType.indexOf('vc_row')===0&&_currentData.name==='columns'){self.__updateColumnsLayout(_currentData.id,_currentData.value)}
