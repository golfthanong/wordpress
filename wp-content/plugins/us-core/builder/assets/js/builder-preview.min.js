...
self.postMessage('navigator.showPreloader',elmId);self.postMessage('builder.elmDuplicate',elmId)},__removeClassInCopiedElm:$ush.debounce($ush.fn,1000*4),_elmCopy:function(e){var self=this,$target=$(e.currentTarget),$highlight=$target.closest('.usb-hover'),elmId=$highlight.data('elmid');if(!elmId||!$usb.builder.isRow(elmId)){return}
self.postMessage('builder.elmCopy',elmId);$target.addClass('copied');self.__removeClassInCopiedElm(function(){$target.removeClass('copied')})},_elmDelete:function(e){var self=this,$highlight=$(e.currentTarget).closest('.usb-hover'),elmId=$highlight.data('elmid');if(!elmId){return}
